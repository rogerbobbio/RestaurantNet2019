...
namespace RestaurantNet
{
  public static class ConvertUtil
  {
    public static Byte[] ToArray(Stream stream)
    {
      ...
      bytesRead = stream.Read(result, 0, (int)stream.Length);
      return result;
    }
    public static T ChangeType<T>(object value, IFormatProvider provider)
    {
      ...
      if (value is string) value = EmptyStringAsNull(((string)value).Trim());

      return ConvertT<T>.Converter(value, provider);
    }
    public static T ChangeType<T>(object value)
    {
      return ChangeType<T>(value, null);
    }
    public static T ChangeTypeSafe<T>(object value, T defaultValue, IFormatProvider provider)
    {
      try
      {
        return ChangeType<T>(value, provider);
      }
    ...
    public static T ChangeTypeSafe<T>(object value, T defaultValue)
    {
      return ChangeTypeSafe(value, defaultValue, null);
    }
    public static T ChangeTypeSafe<T>(object value, IFormatProvider provider)
    {
      return ChangeTypeSafe(value, default(T), provider);
    }
    public static T ChangeTypeSafe<T>(object value)
    {
      return ChangeTypeSafe<T>(value, null);
    }
    public static string EmptyStringAsNull(string str)
    {
      return str != null && str.Length == 0 ? null : str;
    }
    public static object NullIf<T>(T value, T equivalentValue, object nullValue)
    {
      return value == null || EqualityComparer<T>.Default.Equals(value, equivalentValue) ? nullValue : value;
    }
    public static object NullIf<T>(T value, T equivalentValue)
    {
      return NullIf(value, equivalentValue, null);
    }
    public static object NullIfDefault<T>(T value, object nullValue)
    {
      return NullIf(value, default(T), nullValue);
    }
    public static object NullIfDefault<T>(T value)
    {
      return NullIfDefault(value, (object)null);
    }
    public static object DbNullIf<T>(T value, T equivalentValue)
    {
      return NullIf(value, equivalentValue, Convert.DBNull);
    }
    public static object DbNullIfDefault<T>(T value)
    {
      return DbNullIf(value, default(T));
    }
    ...
    {
      #region Fields
      internal static readonly Func<object, IFormatProvider, T> Converter;
      #endregion

      #region Methods
      static ConvertT()
      {
        Converter = Create(typeof(T));
      }
      private static Func<object, IFormatProvider, T> Create(Type type)
      {
        if (IsNullableType(type))
          return (Func<object, IFormatProvider, T>)Delegate.CreateDelegate(typeof(Func<object, IFormatProvider, T>), typeof(ConvertT<T>).GetMethod("NullableTypeConverter", BindingFlags.NonPublic | BindingFlags.Static).MakeGenericMethod(new Type[] { type.GetGenericArguments()[0] }));

        return GeneralTypeConverter;
      }
      private static TElem GetConvertedValue<TElem>(object value, IFormatProvider provider)
      {
        try
        {
          return (TElem)value;
        ...
        {
          return (TElem)Convert.ChangeType(value, typeof(TElem), provider);
        }
      }
      private static TElem? NullableTypeConverter<TElem>(object value, IFormatProvider provider) where TElem : struct
      {
        if (IsNullOrDbNull(value)) return null;

        return new TElem?(GetConvertedValue<TElem>(value, provider));
      }
      private static T GeneralTypeConverter(object value, IFormatProvider provider)
      {
        if (IsNullOrDbNull(value)) return default(T);

        return GetConvertedValue<T>(value, provider);
      }
      #endregion
    }
    ...
    public sealed class Numalet
    {
      ...
      private static String Convertir(Decimal Numero, Int32 Decimales, String SeparadorDecimalSalida, String MascaraSalidaDecimal, 
                                      Boolean EsMascaraNumerica, Boolean LetraCapital, Boolean ConvertirDecimales, 
                                      Boolean ApocoparUnoParteEntera, Boolean ApocoparUnoParteDecimal)
      {
        Int64 Num;
